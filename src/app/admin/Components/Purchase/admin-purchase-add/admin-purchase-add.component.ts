...
@Component({
  selector: 'app-admin-purchase-add',
  templateUrl: './admin-purchase-add.component.html',
  styleUrls: ['./admin-purchase-add.component.scss']
})
export class AdminPurchaseAddComponent implements OnInit {
  ...
  onSubmit(): void {
    this.purchaseService.addData(this.purchaseForm.value).subscribe((response: any) => {
      ...
      this.router.navigateByUrl('/admin/purchaselist'); // Redirect to purchase list page
    });
  }
